# run_param+run_best, fullbatch+Decoupled
DEV=${1:-0}
SEED_P=1
...
PARLIST="normg,dp_lin,dp_conv,lr_lin,lr_conv,wd_lin,wd_conv"
    ...
    python run_best.py  --data $data --model DecoupledVarCompose --conv AdjConv,ChebConv,BernConv "${ARGS_S[@]}"
...
PARLIST="$PARLIST,beta"
    ...
    python run_best.py  --data $data --model DecoupledFixedCompose --conv AdjiConv,AdjiConv "${ARGS_S[@]}" \
        --theta_scheme ones,ones --theta_param 1,1 --alpha 1.0,-1.0
...
PARLIST="$PARLIST,theta_param"
    ...
    python run_best.py  --data $data --model DecoupledFixedCompose --conv Adji2Conv,Adji2Conv "${ARGS_S[@]}" \
        --theta_scheme gaussian,gaussian --alpha="-1.0,-1.0"
    ...
    python run_best.py  --data $data --model DecoupledFixedCompose --conv AdjDiffConv,AdjDiffConv "${ARGS_S[@]}" \
        --theta_scheme appr,appr --alpha 1.0,1.0
